CREATE OR REPLACE FUNCTION funcClustersComparisonDomain(newerDate DATE, olderDate DATE) RETURNS INTEGER AS $$
DECLARE
...
BEGIN
	...
	(
		...
		WHERE 	table_catalog = CURRENT_CATALOG 
			...
			AND table_name = 'comparetableclustersdomain'
	) THEN

		DROP TABLE compareTableClustersDomain;
	END IF;

	CREATE TABLE compareTableClustersDomain
	(
	...
	CREATE TABLE temp_clusters_domain_reversed (cluster_id INTEGER, domain_name varchar(400));
	INSERT INTO temp_clusters_domain_reversed 
	(
		SELECT 		c.cluster_id, concat(reverseDomain(d.domain_name), '.')
	...
	CREATE TABLE temp_clusters_domain_reversed_old (cluster_id INTEGER, domain_name varchar(400));
	INSERT INTO temp_clusters_domain_reversed_old 
	(
		SELECT 		c.cluster_id, d.domain_name
		...
		IF countDomainsPerCluster <> 0 THEN
			...
			WHILE totalOldClustersIter > 0 LOOP
				...
				IF countDomainsPerClusterOld <> 0 THEN
					...
					--in the older table to the cluster in the newer table
					SELECT count(*) into overlappingDomainCount 
					FROM (
						SELECT 	domain_name 
						FROM 	temp_clusters_domain_reversed
						WHERE 	cluster_id = totalNewClustersIter

						INTERSECT 

						SELECT 	domain_name 
						FROM 	temp_clusters_domain_reversed_old
						WHERE 	cluster_id = totalOldClustersIter

					) AS temp_table;
					
					INSERT INTO compareTableClustersDomain 
					VALUES
					(	
	...
	DROP TABLE temp_clusters_domain_reversed;
	DROP TABLE temp_clusters_domain_reversed_old;
...
SELECT * from funcClustersComparisonDomain('20120417', '20120417');

SELECT * FROM compareTableClustersDomain
WHERE  matching >= 50 order by newerClusterId; 
