 ...
 * domainname based similarity.
 * 
 * @author Chris Neasbitt
 */
public class DomainnameSimilarityCalculatorThread extends SimilarityCalculatorThread {
	...
	private static Log log = LogFactory.getLog(DomainnameSimilarityCalculatorThread.class);
	
	/**
	 * Instantiates a new domainname similarity calculator thread.
	 *
	 ...
	 * 		similarity
	 * @throws IOException if the DomainnameSimilarityCalculatorThread.properties
	 * 		file can not be loaded
	 */
	public DomainnameSimilarityCalculatorThread(DBInterface db, String adatestr,  String bdatestr,
		...
		for (int aclus : aclusters){
			for(int bclus : bclusters){
				...
				try{
					...
					if(result != null && Math.abs(result - 0.0) > 0.00001) {
	...
	/**
	 * Execute the domainname similarity query.
	 *
	...
	private Double executeQuery(int aclus, int bclus) throws SQLException{
		...
		formatter.format(query, adatestr, adatestr, aclus, 
				adatestr, adatestr, aclus, bdatestr, bdatestr, bclus);
